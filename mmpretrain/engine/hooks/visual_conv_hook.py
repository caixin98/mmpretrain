...
class VisualConvHook(Hook):
    ...
    def after_train_iter(self, runner, split_channel=False): # weather split RGB channel for mask and psf 
        ...
        if runner.iter % self.visual_freq == 0 and runner.model.device_ids[0] == 0:
            ...
            conv_weight = runner.model.module.backbone.optical._psf.cpu().detach().numpy()
            mask = runner.model.module.backbone.optical.mask.cpu().detach().numpy()
            if split_channel:
                ...
                runner.model.module.backbone.optical.save_psf(save_psf_path)
