...
from mmpretrain.devices import SensorParam, SensorOptions, sensor_dict
...
class AddPoissonGaussianNoise:
    ...
    def __call__(self, measurement):
        ...
        # with torch.no_grad():
        #     measurement_detach = measurement.clone().detach()
        # min_vals = torch.min(torch.flatten(measurement_detach, start_dim=1), dim=1)[0][:,None,None,None]
    
        # measurement = measurement - min_vals
        # measurement_detach = measurement_detach - min_vals
        # sig_max = torch.max(torch.flatten(measurement_detach, start_dim=1), dim=1)[0][:,None,None,None]
        # light_level = sig_max / self.expected_light_intensity 

        # measurement = measurement / (sig_max + 1e-6)
