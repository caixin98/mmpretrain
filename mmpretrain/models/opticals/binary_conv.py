...
class BinaryPsfConv(PsfConv):
    def __init__(self, feature_size = 3e-5, is_binary = True, mask_kernel_shape = None, **kwargs):
    ...
    mask_dim = None):
        ...
        if self.is_binary:
            bin_psf_vals = self.binarize(psf_vals)
        else:
            bin_psf_vals = psf_vals
        
        bin_psf_vals = bin_psf_vals.squeeze(0)
        # indices = np.where(bin_psf_vals[1].cpu().detach().numpy() == 1)
        # print("small kernel indices", list(zip(indices[0], indices[1])))
        bin_psf_vals = torch.sum(bin_psf_vals, dim=0)
        #print the coordinate of bin_psf_vals achieve max
        # indices = np.where(bin_psf_vals.cpu().detach().numpy() == 2)
        # print("indices", list(zip(indices[0], indices[1])))
        mask = bin_psf_vals.repeat(3, 1).reshape(3,bin_psf_vals.shape[0],bin_psf_vals.shape[1])
        mask = mask.cuda()
        return mask
