...
class AffineFaceImageClassifier(FaceImageClassifier):
    ...
    def loss(self, img, data_samples):
        ...
        #     data_samples[i].affine_matrix = data_samples[i].affine_matrix.cuda()
        affine_matrix = torch.cat([data_sample.affine_matrix for data_sample in data_samples], dim=0).cuda()
        gt_label = torch.cat([data_sample.gt_label for data_sample in data_samples], dim=0).cuda()
